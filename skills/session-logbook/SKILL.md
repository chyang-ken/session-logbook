...
Useful search filters are `--source claude|codex|kimi|antigravity|devin|pi`, `--project <substring>`, `--since 7d`
...
Antigravity `follow` is cursor-based: anchors stay the file's own ascending physical lines,
and the removal list is what makes an append-only cursor safe across a rewind.
