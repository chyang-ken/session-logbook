...
--source-hermes-chip-bg/-fg     /* teal background + deep-teal text: fourth source, Hermes */
--source-*-glow-1/-2          /* the two-layer glow of the modal's rectangular light band (rgba) */
--source-*-solid              /* opaque version of the glow (claude #d9a550 / codex #6491af / antigravity #8b6fc4 / hermes #529b93): color dots / badge outlines, more vivid than the chip */
```
...
**Iron rule**: a given source uses this same palette in every component — warm = Claude / cool = Codex / purple = Antigravity / teal = Hermes, never swapped. For a stronger statement use `-solid` (filled); don't reach for `--gold` / `--accent`.
